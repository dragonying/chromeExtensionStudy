...
const dy = new Dy({ defaultWait: 2 });

dy.getUserProList("MS4wLjABAAAAumvAfskbDW1CB77yOudtt4ecmpi3Mkdm8XCkrTfRZPQ", function (aweme_list) {
    const key = 'proList';
...
function getCurrentAwemeId(e) {
    return e.target.parentNode.parentNode.querySelector('.video-info-detail').getAttribute('data-e2e-aweme-id');
}

function addToolBox() {
    document.querySelectorAll('#slidelist .playerContainer').forEach(o => {
        if (o.querySelector('.zfyTool')) {
            return;
        }
        let toolBox = document.createElement("div");
        toolBox.className = "zfyTool";
        let downObj = document.createElement("Button");
        downObj.innerText = "下载无水印素材";
        downObj.onclick = async function (e) {
            let aweme_id = getCurrentAwemeId(e);
            // const { aweme_detail: { video: { play_addr: { url_list } } } } = await dy.proDetail(getCurrentAwemeId(e));
            let url_list = [];
            e.target.parentNode.parentNode.querySelectorAll('video source').forEach(o => url_list.push(o.getAttribute('src')));
            let link = url_list.pop();
            window.open(link, '_blank')
            // let domA = document.createElement('a');
            // domA.setAttribute('download', `${aweme_id}.mp4`);
            // domA.setAttribute('href', link);
            // domA.click();
            // setTimeout(() => { document.removeChild(link) }, 2000);
        }
        let commentUser = document.createElement("Button");
        commentUser.innerText = "采集视频评论区用户";
        commentUser.onclick = function (e) {
            dy.getProCommentList(getCurrentAwemeId(e));
        }
        let authorPro = document.createElement("Button");
        authorPro.innerText = "下载作者所有作品";
        authorPro.onclick = function () {
            dy.getUserProList();
        }
        toolBox.appendChild(commentUser);
        toolBox.appendChild(downObj);
        toolBox.appendChild(authorPro);
        o.appendChild(toolBox);
    });
}
